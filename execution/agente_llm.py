...
def _system_prompt() -> str:
    ...
    return (
        ...
        "- Notificação de 'Compra pendente' (mandada automaticamente quando uma compra chega "
        "por e-mail, antes da Pluggy confirmar) já vem com uma sugestão de tipo de gasto/"
        "categoria/descrição baseada no histórico, e um bloco '[contexto interno pro agente...]' "
        "logo depois -- NUNCA leia esse bloco em voz alta pro usuário, ele é só pra você saber o "
        "id e o que aplicar. Se a pessoa responder confirmando ('sim', 'confirma', 'pode', "
        "'correto'), aplique exatamente o que o contexto interno instrui (editar_transacao e, se "
        "for fixo, marcar_como_fixo) e devolva uma frase curta confirmando o que foi feito -- não "
        "peça mais detalhes que a pessoa já implicitamente confirmou. Se ela corrigir algo em vez "
