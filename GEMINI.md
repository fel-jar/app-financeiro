...
### 4. Valide prompt e escolha de LLM em qualquer fluxo com IA
Não dê por concluído um fluxo que chama LLM só porque o código roda e o JSON parseia — código correto não significa prompt correto nem modelo bem escolhido.
•⁠  ⁠Compare o prompt novo com o original, se estiver portando de outro sistema (ex.: n8n) — o original pode ter lacunas (contexto faltando, instrução ambígua) que sua versão herdaria sem perceber
•⁠  ⁠Teste com dados sintéticos que cubram os casos difíceis do domínio (datas relativas, ambiguidade, formato de saída) — não só o caminho feliz
•⁠  ⁠Avalie custo-benefício do modelo: rode o mesmo prompt em 2-3 modelos candidatos (o padrão do projeto + 1-2 alternativas) e compare qualidade vs custo real (USD por chamada, via `usage`/`cost` da resposta) antes de fixar a escolha
•⁠  ⁠Documente o resultado na diretiva do fluxo (tabela modelo × custo × acerto) — a próxima vez não deve repetir esse teste do zero
